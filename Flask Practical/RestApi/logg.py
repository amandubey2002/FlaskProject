...
handler = TimedRotatingFileHandler(
    os.path.join(log_dir, f"{today_date}error.log"), when="midnight", backupCount=7
)
...
handler1 = TimedRotatingFileHandler(
    os.path.join(log_dir, f"{today_date}debug.log"), when="midnight", backupCount=7
)
...
handler2 = TimedRotatingFileHandler(
    os.path.join(log_dir, f"{today_date}info.log"), when="midnight", backupCount=7
)
